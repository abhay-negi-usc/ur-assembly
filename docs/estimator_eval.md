...
| `eval.trajectory_noise` | optional smoothed per-waypoint Gaussian noise in the connector's own frame, redrawn per attempt; `std` sets the **per-dimension** std `[x,y,z (m), r,p,y (deg)]` (zero = DOF untouched; legacy scalar keys broadcast); `noise_decay_attempt` shrinks the whole perturbation `(1−f)^(attempt−1)`, `noise_decay_traj` sheds it linearly along the path (full → ×(1−f) at the last waypoint), `alternate_pitch_deg` adds an initial pitch offset with per-attempt alternating sign; its own random stream, so the injected-error draws are unchanged. The same block (identical semantics) is available in `cable_pick_estimate_assemble` under `assembly.trajectory_noise` |
