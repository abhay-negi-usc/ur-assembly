...
def test_solution_check_scores_and_gate():
    """The check must compute BOTH scores, gate via the configured method, keep the belief
    on skip_correction, pass through on warn, and NEVER flag without calibration."""
    import csv as _csv
    ...
    try:
        ...
        cfg = {'manifold_csv': path, 'estimate_dims': ['z_mm', 'pitch_deg'],
               ...
               'check': {'enabled': True, 'method': 'cauchy', 'flag_threshold': 0.99,
                         'on_flag': 'skip_correction', 'n_candidates': 32,
        ...
        # threshold below any score -> skip_correction must KEEP the belief (None + reason)
        est.flag_threshold = -1.0
        none_corr, reason = est.estimate(v6, w6)
        assert none_corr is None and 'check flagged' in reason, reason
        # ... while on_flag: warn applies the correction and marks the flag
        est.on_flag = 'warn'
        T2, info2 = est.estimate(v6, w6)
        assert T2 is not None and info2['check']['flagged'] is True
        # method selection: rankavg2 must gate on ITS score
        est.check_method = 'rankavg2'
        none3, reason3 = None, ''
        est.on_flag = 'skip_correction'
        none3, reason3 = est.estimate(v6, w6)
        assert none3 is None and 'rankavg2' in reason3, reason3

        # WITHOUT calibration: signals logged, scores None, NEVER flags (even at -1)
        cfg2 = dict(cfg)
        ...
        T3, info3 = est2.estimate(v6b, w6b)
        assert T3 is not None and info3['check']['score'] is None
        assert info3['check']['flagged'] is False
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
...
def test_check_app_config_is_wired():
    ...
    assert str(chk['on_flag']) in ('skip_correction', 'warn')
