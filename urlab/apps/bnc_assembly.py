...
def build_and_run(cfg, robot, camera, args):
    ...
    def reorient_recovery():
        ...
        # THE SQUARE GRASP, for this manoeuvre only -- restored in the finally, so a recovery
        # that fails half way cannot leave the run picking with the wrong geometry.
        keep = cfg.get_path('pickup.fingertip_in_connector')
        square = dict(keep or {})
        square['rpy_deg'] = [float(v) for v in (r.get('grasp_rpy_deg') or [0.0, 0.0, 0.0])]
        try:
            cfg.set_path('pickup.fingertip_in_connector', square)
            log.info('REORIENT: picking SQUARE (rpy %s) instead of the coaxial %s.',
                     square['rpy_deg'], (keep or {}).get('rpy_deg'))
            phase('scan')
            if _pick(cfg, robot, scanner, geom, check, recovery, grasp, confirm,
                     recorder) != 'ok':
