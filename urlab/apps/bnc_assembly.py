...
def build_and_run(cfg, robot, camera, args):
    ...
    def collar_clocking(T_base_conn, screw_deg=None):
        ...
        (log.info if ftip_lat < 1e-3 else log.warning)(
            '  fingertip sits %.2f mm off the connector axis while gripping. The pitch pivots '
            'THERE, so this is exactly how far the grasp will land off the ring%s.',
            ftip_lat * 1000.0,
        ...
        # THE PITCH ITSELF: the signed angle about the connector -Y that carries tool0 +Z onto
        # the connector +X, and the pose it produces when applied about the fingertip.
        u_pitch = -T_clk[:3, 1]
        u_pitch = u_pitch / float(np.linalg.norm(u_pitch))
        z_now = T_withdraw[:3, 2]
        pitch_rad = float(np.arctan2(float(np.dot(np.cross(z_now, axn), u_pitch)),
                                     float(np.dot(z_now, axn))))
        T_reorient = rotate_about_axis(T_withdraw, u_pitch, pivot, pitch_rad)

        # A rotation about a FIXED axis can only carry z_now exactly onto the axis when the two
        # make the same angle with it. They do here (both are perpendicular to the connector Y),
        # but the frames are edited by hand, so the residual is measured rather than assumed.
        resid_deg = float(np.degrees(np.arccos(np.clip(
            float(np.dot(T_reorient[:3, 2] / np.linalg.norm(T_reorient[:3, 2]), axn)),
            -1.0, 1.0))))
        log.info('  pitch %+.2f deg about the connector -Y, pivoting on the fingertip -> tool0 +Z '
                 'lands %.4f deg off the connector +X.', np.degrees(pitch_rad), resid_deg)
        if resid_deg > 0.5:
            log.error('COLLAR CLOCKING: a pitch about the connector -Y cannot lay tool0 +Z on the '
                      'connector +X from here -- %.2f deg of residual. That means the held '
                      'attitude is not square to the connector Y, so no single pitch reaches the '
                      'axial pose. Check frames.yaml fingertip and the connector frame rpy.',
                      resid_deg)
            return False

        # Where the fingertip now stands on the axis, and therefore how long the threading leg is.
        ftip_x = float((inverse(T_base_axis) @ T_reorient @ robot.T_tool0_fingertip)[0, 3])
        adv_m = collar_x - ftip_x             # the guarded axial leg that threads the cable
        T_grip0 = translation_matrix(adv_m * axn) @ T_reorient      # the grasp at roll 0
        ...
        # ---- 2. PITCH ONTO THE AXIS, about the fingertip --------------------------------------
        # One move_j: the whole reorientation, with the fingertip standing still. A joint move
        # rather than a straight line because it is the only large reorientation in the maneuver.
        # NOTE the fingers are still around the cable and that is FINE here -- the pitch axis is
        # the jaw-closing axis, so the jaw turns about its own gap and the cable in it is at the
        # pivot. There is no lateral lift-off any more; nothing needs clearing.
        #
