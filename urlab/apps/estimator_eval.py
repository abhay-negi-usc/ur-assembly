...
def build_and_run(cfg, robot, camera, args):
    ...
    # Per-attempt convergence plots are OPTIONAL (they multiply across N x M attempts). The plot
    # helper lives with the pick app; imported lazily so this bench app stays free of the
    # scan/pick stack unless plots are actually requested.
    plot_fn = None
    if bool(ev.get('save_plots', False)):
        try:
            from .cable_pick_estimate_assemble import _plot_estimate
            plot_fn = _plot_estimate
        except Exception as exc:                   # noqa: BLE001 -- plots are never fatal
            log.warning('eval.save_plots disabled (%s)', exc)
                ...
                if T_corr_mm is None:
                    ...
                    if plot_fn is not None:
                        plot_fn(os.path.join(
                            out_dir, f'trial_{trial:03d}_attempt_{attempt:02d}_estimate.png'),
