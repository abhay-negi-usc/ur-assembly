...
def resolve_device(requested):
    ...
    'auto'  -- cuda when available, else cpu. The right setting for configs that move between
               the GPU robot host and CPU-only laptops.
    ...
    'cuda'  -- require the GPU: raise AT CONSTRUCTION if it is missing, instead of crashing
               ...
               CPU-only laptop are the reason this knob exists.
    """
    import torch
    ...
    if req == 'cuda':
        if not torch.cuda.is_available():
            raise RuntimeError(
                "compute.device is 'cuda' but torch reports no CUDA device "
                "(torch %s). Set compute.device: auto|cpu in configs/_common.yaml, or install "
                "a CUDA torch build." % torch.__version__)
        return 'cuda'
...
class Sam3Backend:
    ...
    def __init__(self, cable_prompt="cable", connector_prompt="connector",
        ...
        # ONE resolution, up front, from config -- everything below keys off self.device. The
        # hardcoded-cuda crashes this replaces surfaced one at a time, layers deep.
