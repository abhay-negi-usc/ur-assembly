"""BNC assembly BEHAVIOURS -- the functions the algorithm in apps/bnc_assembly.py calls.

Each behaviour takes the run's shared state (`asm`, a domain.BncAssembly) plus its own explicit
arguments, does one nameable thing, and returns a status the algorithm can branch on. No behaviour
reads the yaml directly: tuning arrives through asm.spec. Grown one function per extraction step
until the app file is nothing but the algorithm.
"""
...
from ..apps._cable import build_scanner, make_confirm
from ..apps._common import guarded, prompts_off
from ..apps.cable_pick_assemble import _pick
from ..domain import (clocking_plan as _clocking_plan,
                      compliance_override as _compliance_override,
                      fit_turn as _fit_turn, path_time,
                      retreat_state as _retreat_state,
                      wrap_near as _wrap_near, wrist3_window as _wrist3_window)
from ..robot import AdmittanceController, ForceGuard
from ..robot.detectors import (AnyGuard as _AnyGuard, AxialForce as _AxialForce,
                               RadialConfirm as _RadialConfirm,
                               ScrewAdvance as _ScrewAdvance,
                               TravelReached as _TravelReached)
from ..transforms import (UR_JOINTS, from_cfg, inverse, matrix_to_xyzrpy,
                          pose_error,
                          rotate_about_axis, slerp_matrix, translation_matrix,
                          xyzrpy_to_matrix)
from . import reset
from .estimate import (corr_to_m as _corr_to_m, observe,
                       observe as _observe,
                       save_observations as _save_observations)
from . import marker_localize as mloc
from .pick import (GraspCheck, GraspController, GraspGeometry,
                   GraspImageRecorder, GraspRecovery, belief_offset,
                   connector_axis_height_m, fingertip_in_connector,
                   held_belief, offset_belief)
from . import trajectory as traj
from . import wiggle as wigmod
from .insert import retract_ref
from .solution_check import CheckedManifoldEstimator
from .manifold import mats_from_vec6, vec6_from_mats

log = urlog.get('bnc-behaviors')
...
def traj_ref(asm, row, T_tool0_conn, commit=False):
    """A TRAJECTORY row -> a tool0 reference, anchored so the path ends on the mate.

    ANCHORED (frames_t.T_base_targetobj = the mate with the CSV's last row normalised away), which is
    ...
    frames_t = asm.frames
    return ((frames_t.T_base_commit if commit else frames_t.T_base_targetobj)
            @ row @ inverse(T_tool0_conn))


def engage_insertion(asm):
    ...
    robot, frames_t = asm.robot, asm.frames
    adm_en, guard_en, guard_shared = asm.adm_en, asm.guard_en, asm.guard
    tare, out_dir, dense = asm.tare, asm.out_dir, asm.dense
    phase, scales = asm.pace.phase, asm.pace.scales
    g_v, s_eng = asm.pace.g_v, asm.pace.s_eng
    _guarded = guarded
    _es = asm.spec.engage
    en_wig, en_amp, en_frq, en_scale, en_pv, en_pw = asm.engage_rig
    en_rate = float(_es.sample_rate_hz)
    en_travel_mm = float(_es.travel_mm)
    en_timeout_s = float(_es.timeout_s)
    en_align_s = float(_es.align_s)
    en_pre_mm = float(_es.preload_mm or 0.0)
    en_fmax = float(_es.max_axial_force_n or 0.0)
    en_fpers = float(_es.persistence_s or 0.0)
    ct_on = bool(_es.contact.enabled)
    ct_force_n = float(_es.contact.force_n)
    ct_pers_s = float(_es.contact.persistence_s)
    cf_on = bool(_es.confirm.enabled)
    cf_radial_n = float(_es.confirm.radial_force_n)
    cf_radial_s = float(_es.confirm.radial_persistence_s)
    cf_max_mm = float(_es.confirm.max_wiggle_mm)
    cf_cycles = max(1, int(_es.confirm.cycles))
    cf_fallback_s = float(_es.confirm.fallback_s)
    decim = max(1, int(asm.spec.run.log_decimation))
    settle_shared = asm.settle_s
    ...
    if en_pre_mm > 0.0:
        step = float(asm.spec.trajectory.translational_resolution_mm)
    ...
    def ref_at(t, dur=None):
        ...
        return traj_ref(asm, T, T_tool0_conn)
    ...
    first = ref_at(0.0)
    phase('standoff')
    q = robot.arm.ik(first, asm.seed_q)
    ...
    def _conn_x_mm(T_base_tool0):
        ...
        if T_base_tool0 is None:
            return float('nan')
        rel = inverse(frames_t.T_base_tconn) @ (T_base_tool0 @ T_tool0_conn)
    ...
    def log_cb():
        ...
        if cnt[0] % decim != 0:
            return
        row = _observe(robot, T_tool0_conn, frames_t.T_base_tconn)
        obs.append(row)
        d = adm_en.delta
        flange = robot.tool0()
        trace.append([
    ...
    if ct_on:
        ...
        while True:
            ...
            if tc >= ct_dur:
                break
            d = ct_v * tc
            cur = traj_ref(asm, _corr_to_m(mats_from_vec6(path_at(d))), T_tool0_conn)
        ...
        if ct_status != 'contact':
            ...
            depth0 = float(matrix_to_xyzrpy(
                inverse(frames_t.T_base_tconn) @ (robot.tool0() @ T_tool0_conn))[0][0] * 1000.0)
        ...
        if not phase_gate(asm,  # prompt that may sit for minutes
                'ENGAGE (from contact)',
    ...
    T_meas = inverse(frames_t.T_base_targetobj) @ (robot.tool0() @ T_tool0_conn)
    ...
    if status in ('force', 'travel') and cf_on:
        ...
        while True:
            ...
            cur_c = traj_ref(asm, Tc, T_tool0_conn)
...
def setup(asm):
    """CONSTRUCT AND VALIDATE THE RUN -- everything that happens before the arm moves.

    Reads the yaml through asm.spec (plus the raw blocks the legacy schema still carries),
    refuses bad configuration BEFORE any motion, builds every controller/estimator/scanner the
    behaviours share, anchors the target frames, and creates the experiment folder. Publishes
    all of it on the bag; returns False on a configuration the run must not start from."""
    robot, camera, cfg, spec = asm.robot, asm.camera, asm.cfg, asm.spec
    # The startup narration keeps the app's voice: these lines are the run's construction
    # record, and the golden dry-run trace pins them under the 'bnc-assembly' logger name.
    ...
    asm.scanner, asm.geom, asm.check = scanner, geom, check
    asm.recovery, asm.recorder, asm.confirm = recovery, recorder, confirm
    ...
    adm = asm.adm = AdmittanceController(robot.arm, cfg.section('compliance'))
    guard_shared = ForceGuard(robot.arm, cfg.section('force_guard'))
    asm.guard = guard_shared
    tare = (lambda: robot.arm.zero_ft(settle=False)) \
        if bool(cfg.get_path('compliance.tare_before', True)) else None
    asm.tare = tare
    asm.settle_s = float(cfg.get_path('compliance.settle_s', 0.5))
    asm.hold_s = float(cfg.get_path('compliance.hold_after_insertion_s', 0.0))

    # ---- FINAL INSERTION: one grouped block, each key inheriting the shared value when null.
    _sfi = spec.final_insertion
    _fic = _sfi.compliance or {}
    _fig = _sfi.force_guard or {}
    ...
    fi_preload_mm = float(_sfi.preload_mm or 0.0)
    ...
    for nm, v in (('speed_translation_mm_s', fi_v), ('speed_rotation_deg_s', fi_wr),
        ...
        if v is not None and v < 0:
            log.error('final_insertion.%s must be >= 0 (got %.2f).', nm, v)
            return False
    adm_final, guard_final = adm, None
    if fi_on:
        comp_final = dict(cfg.section('compliance'))
        for k in ('stiffness', 'mass', 'damping_ratio'):
            if _fic.get(k) is not None:
                comp_final[k] = [float(v) for v in _fic[k]]
        adm_final = AdmittanceController(robot.arm, comp_final)
        gsec = dict(cfg.section('force_guard'))
        over_g = {k: _fig[k] for k in ('max_force_n', 'max_torque_nm', 'persistence_s')
                  if _fig.get(k) is not None}
    ...
    if ins_mode not in ('estimate', 'engage'):
        log.error("run.insertion_mode %r must be 'estimate' or 'engage'. The old\n"
                  ...
                  "engage.amplitude instead.", ins_mode)
        return False
    # ---- ENGAGE: the trajectory-following insertion ------------------------------------------
    # TUNING COMES FROM THE SPEC. BncSpec (urlab/domain.py) parses the whole behaviour's block
    # once -- defaults live on the dataclass, unknown keys are rejected -- and these locals are
    # just its fields under the names the rest of the file already uses. `en` (the raw block)
    # survives only for _clock_physics until the compliance bank lands.
    _es = spec.engage
    en_amp = [float((_es.amplitude or {}).get(d, 0.0)) for d in DIM_KEYS]
    en_frq = [float((_es.frequency_hz or {}).get(d, 0.0)) for d in DIM_KEYS]
    en_rate = float(_es.sample_rate_hz)
    ...
    en_pre_mm = float(_es.preload_mm or 0.0)
    # RETIRED: engage.speed_mm_s. The engage reference rate is now speed.phase_scale
    ...
    en_speed_retired = _es.speed_mm_s
    en_fmax = float(_es.max_axial_force_n or 0.0)
    ...
    _eblk, _esrc = wigmod.from_shared(cfg, _es.wiggle, _es.wiggle_from, 'engage')
    en_wig = None
    try:
        ...
        if en_wig is not None:
            en_wig.validate(rate_hz=en_rate,
                            ...
                            or _es.max_oscillation_speed_mm_s,
                            cap_w=_eblk.get('max_rotation_deg_s')
                            or _es.max_oscillation_rotation_deg_s)
    ...
    en_scale, en_pv, en_pw, en_orbit = traj.wiggle_time_scale(
        en_amp, en_frq, _es.max_oscillation_speed_mm_s, _es.max_oscillation_rotation_deg_s)
    asm.engage_rig = (en_wig, en_amp, en_frq, en_scale, en_pv, en_pw)
    if ins_mode == 'engage':
        if en_rate <= 0:
            log.error('engage.sample_rate_hz must be > 0.')
            return False
        if en_pre_mm < 0:
            log.error('engage.preload_mm must be >= 0.')
            return False
        if en_speed_retired is not None:
            ...
            log.error(
                'engage.speed_mm_s is RETIRED -- the engage reference rate now comes '
        ...
        for i, d in enumerate(DIM_KEYS):
            if abs(en_amp[i]) > 0.0 and en_frq[i] <= 0.0:
                log.error('engage.frequency_hz.%s must be > 0 when its amplitude is '
        ...
        if f_live and en_rate < 4.0 * max(f_live):
            log.error('engage.sample_rate_hz %.1f Hz is too coarse for a %.2f Hz '
                      'component (need >= 4x) -- the sampled sine would alias.',
                      en_rate, max(f_live))
            return False
        if en_fmax <= 0:
            log.warning('engage.max_axial_force_n is 0 -- the engage will run the whole '
    ...
    # engaged pose. The frame it rolls (frames_t.T_base_tconn) is built where the target is loaded.
    eng_clock = np.radians(spec.run.engage_clock_deg)
    ...
    cc = {'enabled': spec.connector_clocking.enabled}
    cl = {'enabled': spec.collar_clocking.enabled}
    if cfg.get('assembly') is not None:
        # NOT a warning: with the whole block ignored, every behaviour would run on defaults
        # and the run would head for the socket with none of the operator's tuning.
        log.error('this config still carries the LEGACY assembly: block -- the schema moved to '
                  'per-behaviour top-level blocks (run:, engage:, connector_clocking:, ...). '
                  'See docs/bnc_config_migration.md for the exact mapping.')
        return False

    try:
        cc_on, cl_on = _clocking_plan(cc, cl)
        asm.cc_on, asm.cl_on = cc_on, cl_on
    ...
    _scc, _scl = spec.connector_clocking, spec.collar_clocking
    _sw = _scc.sweep_deg
    if _sw is None:
        cc_sweep = [eng_clock + np.radians(_scc.rotation_deg)]
    else:
        try:
            cc_sweep = [np.radians(float(v)) for v in _sw]
        except (TypeError, ValueError):
            log.error('connector_clocking.sweep_deg must be a list of numbers (roll angles '
                      'in deg wrt the target frame), got %r.', _sw)
            return False
        if not cc_sweep:
            log.error('connector_clocking.sweep_deg is empty -- give it at least one roll '
    ...
    if cc_hi - cc_lo > 2.0 * np.pi:
        log.error('connector_clocking.sweep_deg spans %.0f deg from the engaged roll '
                  '(%+.1f deg) -- more than one turn, so a measured clock angle cannot be told '
                  'from itself plus 360. Narrow the sweep or move run.engage_clock_deg.',
                  np.degrees(cc_hi - cc_lo), np.degrees(eng_clock))
        return False
    asm.cc_geometry = (cc_sweep, cc_legs, cc_lo, cc_hi, cc_mid)
    cc_need_m = float(_scc.success_advance_mm) / 1000.0
    cc_tries = max(1, int(_scc.max_tries))
    cc_open_after = bool(_scc.open_gripper_after)
    cl_axis_off = [float(v) for v in (_scl.axis_offset_mm or [0.0, 0.0, 0.0])]
    if len(cl_axis_off) != 3:
        log.error('collar_clocking.axis_offset_mm must have 3 entries (connector-frame '
                  'xyz, mm), got %r.', cl.get('axis_offset_mm'))
        return False
    _stv = spec.tug_verify
    tv_on = bool(_stv.enabled)
    ...
    _tv_phys = _compliance_override(cfg.section('compliance'), _stv.compliance or {})
    asm.adm_tug = (AdmittanceController(robot.arm, _tv_phys)
                   if tv_on else None)
    asm.ground_z = float(cfg.get_path('ground_plane.z_m', -0.76))
    # ---- DISASSEMBLY (optional) -----------------------------------------------------------
    _sd = spec.disassembly
    dis_on = bool(_sd.enabled)
    dis_place_on = bool(_sd.place.enabled)
    ...
    if dis_on and not cc_on:
        ...
        log.error('disassembly.enabled needs connector_clocking enabled -- the '
                  'disassembly reverses the clocking, and without it there is nothing to '
                  'reverse and no engaged pose to reverse from.')
        return False

    _ssp = _scl.seat_push
    sp_on = bool(_ssp.enabled)
    sp_force = float(_ssp.force_n)
    sp_persist = float(_ssp.persistence_s)
    ...
    asm.guard_push = guard_push
    if cc_on and cl_on and cc_open_after:
        log.error('collar_clocking needs connector_clocking.open_gripper_after FALSE: '
    ...
    if cc_on and cc_tries > 1 and len(cc_sweep) < 2:
        ...
        log.warning('connector_clocking: max_tries is %d but the sweep has only ONE roll '
    ...
    if cc_on and cc_need_m <= 0.0:
        log.error('connector_clocking.success_advance_mm must be > 0 (got %.2f) -- a zero '
    ...
    if cc_on:
        adm_cc, guard_cc = _domain.clock_physics(robot.arm, cfg.section('compliance'), cfg.section('force_guard'), _scc.compliance, _scc.force_guard, None, 'Connector clocking', log)
    if cl_on:
        adm_cl, guard_cl = _domain.clock_physics(robot.arm, cfg.section('compliance'), cfg.section('force_guard'), _scl.compliance, _scl.force_guard, None, 'Collar clocking', log)
    asm.adm_cc, asm.guard_cc = adm_cc, guard_cc
    asm.adm_cl, asm.guard_cl = adm_cl, guard_cl
    ...
    if ins_mode == 'engage':
        ...
        # the engage block's own axial persistence doubles as its guard persistence override
        _en_guard = dict(_es.force_guard or {})
        if _es.persistence_s is not None:
            _en_guard.setdefault('persistence_s', _es.persistence_s)
        adm_en, guard_en = _domain.clock_physics(robot.arm, cfg.section('compliance'), cfg.section('force_guard'), _es.compliance, _en_guard, _es.max_axial_force_n, 'Engage insertion', log)
        asm.adm_en, asm.guard_en = adm_en, guard_en

    # ---- Target from the SHARED catalogue (the same record estimator_eval assembles to) ----
    tname = asm.tname = spec.run.target_frame
    targets = asm.targets = tool_frames.load_targets(cfg)
    if not tname or tname not in targets:
        log.error('run.target_frame %r needs a targets: entry in %s.',
                  tname, tool_frames.frames_path(cfg))
        return False
    _str = spec.trajectory
    csv_in = urconfig.resolve(cfg, _str.csv)
    mats = traj.load_csv(csv_in, angles_deg=bool(_str.angles_deg))
    asm.dense = traj.resample(mats, float(_str.translational_resolution_mm) / 1000.0,
                              float(_str.rotational_resolution_deg))
    frames_t = asm.frames = _domain.TargetFrames(R_clock, mats, fi_preload_mm)
    frames_t.anchor(targets[tname])

    # ---- WHERE THE TARGET COMES FROM: run.target_source -----------------------------------
    ...
    if tgt_source not in ('kinematic', 'visual'):
        log.error("run.target_source must be 'kinematic' or 'visual', got %r.",
                  spec.run.target_source)
        return False
    vt_rig = None
    if tgt_source == 'visual':
        ...
        try:
            ...
            log.error('run.target_source is visual but %s', exc)
            return False
        vt_rig = asm.vt_rig = rigs.get(tname)
        if vt_rig is None:
            log.error('run.target_source is visual but %s has no marker_rigs: entry for %r '
    ...
    # the app is built from frames_t.T_base_tconn, so rolling it rolls the trajectory, stand-off,
    ...
    # target-frame leg and the tug all build their axes and stations from frames_t.T_clk. 'target' binds it
    ...
    if pe_frame not in ('target', 'believed'):
        log.error("run.post_engage_frame must be 'target' or 'believed', got %r.",
                  spec.run.post_engage_frame)
        return False
    ...
    _sh_m, _sh_r = pose_error(frames_t.T_base_targetobj, frames_t.T_base_tconn)
    if _sh_m * 1000.0 > 1e-6 or np.degrees(_sh_r) > 1e-6:
        log.warning('trajectory_csv\'s last row is NOT identity (off by %.2f mm / %.2f deg) -- '
                    ...
                    'in final_insertion.preload_mm, not in a trajectory row.',
                    _sh_m * 1000.0, np.degrees(_sh_r))

    # ---- COLLECTION MODE (estimator_eval.eval.collection semantics) ----
    col_mode = str(spec.collection.mode).strip().lower()
    if col_mode not in ('attempts', 'offset_sweep', 'peck'):
        log.error("collection.mode %r must be 'attempts', 'offset_sweep' or 'peck'.",
                  col_mode)
        return False
    sweep_offsets = spec.collection.sweep_offsets
    ...
    if col_mode == 'offset_sweep' and (not sweep_offsets
        ...
        log.error('collection.sweep_offsets must be a non-empty list of 6-vectors '
                  '[x, y, z (m), roll, pitch, yaw (deg)].')
        return False
    peck_back_m = float(spec.collection.peck_retract_mm) / 1000.0
    peck_timeout = float(spec.collection.peck_timeout_s)
    ...
    _stn = spec.trajectory_noise
    tn_std = [float(v) for v in (_stn.std or [0.0] * 6)]
    if len(tn_std) != 6:
        log.error('trajectory_noise.std must have 6 entries.')
        return False
    ...
    _ps_seed = spec.disassembly.place_scatter.seed
    _place_rng = [np.random.default_rng(None if _ps_seed is None else int(_ps_seed))]
    asm.place_rng = _place_rng

    # Pacing lives in the domain now; the aliases keep every existing call site.
    asm.pace = _domain.Pacing(cfg, robot.arm, 1.0 / adm.rate)




    dbg = spec.run.debug_match or {}
    ...
    if init_name:
        if init_name not in frames:
            ...
            return False
        asm.T_ftip_conn = inverse(robot.T_tool0_fingertip) @ frames[init_name]
        ...
        if init:
            ...
            d_lin, d_ang = pose_error(from_cfg(init), asm.T_ftip_conn)
        ...
        asm.T_ftip_conn = from_cfg(init) if init else from_cfg(cfg.section('junction_in_fingertip'))
    ...
    asm.T_ftip_conn_catalogue = np.array(asm.T_ftip_conn, dtype=float)
    asm.T_ftip_conn = held_belief(asm.T_ftip_conn, from_cfg(cfg.section('junction_in_fingertip')),
                              _grip_off)
    ...
    if float(np.linalg.norm(_bo_xyz)) > 0.0 or float(np.linalg.norm(_bo_rpy)) > 0.0:
        asm.T_ftip_conn = offset_belief(asm.T_ftip_conn, _bel_off)
    ...
    no_prompts = asm.no_prompts = prompts_off(cfg)
    if no_prompts:
    ...
    # run.gate_between_behaviors: false to turn them off in config. Dry runs never prompt.
    gates_on = bool(spec.run.gate_between_behaviors) and not no_prompts
    asm.gates_on = gates_on
    ...
    # ---- what the ALGORITHM reads, published on the bag ---------------------------------
    asm.estimator, asm.commit = estimator, commit
    asm.adm_final, asm.guard_final = adm_final, guard_final
    asm.noise_rng = noise_rng
    asm.col_mode, asm.sweep_offsets = col_mode, sweep_offsets
    asm.live_path, asm.mats = live_path, mats
    asm.debug_match = (dbg, dbg_on, dbg_live)
    return True


def disassembly(asm, state, screw_deg, T_base_conn_d):
    ...
    socket frame's own +X through its origin (frames_t.T_clk), which is what connector_clocking
    ...
    Returns (ok, state)."""
    robot, frames_t, clock_rows = asm.robot, asm.frames, asm.clock_rows
    adm_cl, guard_shared = asm.adm_cl, asm.guard
    _guarded = guarded
    phase, seg_time = asm.pace.phase, asm.pace.seg_time
    ground_z = asm.ground_z
    _sd, _scl = asm.spec.disassembly, asm.spec.collar_clocking
    cl_axis_off = [float(v) for v in (_scl.axis_offset_mm or [0.0, 0.0, 0.0])]
    cl_rot = np.radians(_scl.rotation_deg)
    dis_unlock = bool(_sd.unlock_collar)
    dis_extract_m = float(_sd.extract_mm) / 1000.0
    dis_place_on = bool(_sd.place.enabled)
    dis_clear_m = float(_sd.place.clearance_mm) / 1000.0
    dis_rise_m = float(_sd.place.retreat_mm) / 1000.0
    axn_d = frames_t.T_clk[:3, 0] / float(np.linalg.norm(frames_t.T_clk[:3, 0]))
    ...
    if dis_unlock and state in ('locked', 'seated'):
        if not phase_gate(asm, 'UNLOCK',
                          ...
                          % np.degrees(cl_rot)):
            return False, state
        if not collar_clocking(asm, T_base_conn_d, screw_deg,
                                          cl_rot=-cl_rot, sp_on=False,
                               unlocking=True):
    ...
    T_place = aligned_place_pose(asm, dis_clear_m)
    for _try in range(8):
        s = place_scatter(asm)
        if s == (0.0, 0.0, 0.0):
            break                                   # scatter off: the centre IS the pose
        T_try = aligned_place_pose(asm, dis_clear_m, scatter=s)
        if robot.arm.dry_run or robot.arm.ik(T_try @ inverse(asm.T_ftip_conn)) is not None:
            T_place = T_try
        ...
        log.warning('PLACE SCATTER: no reachable draw in 8 tries -- placing at the '
                    'un-scattered centre instead. Reduce disassembly.place_scatter.')
    if not phase_gate(asm, 'PLACE THE CABLE',
    ...
    T_ftip_place = T_place @ inverse(asm.T_ftip_conn)
    ...
    log.info('PLACE: laying the connector along the socket axis -- heading %+.1f deg '
             ...
             np.degrees(np.arctan2(frames_t.T_base_tconn[1, 0], frames_t.T_base_tconn[0, 0])),
...
def connector_clocking(asm):
    ...
    robot, frames_t, clock_rows = asm.robot, asm.frames, asm.clock_rows
    adm_cc, guard_cc, phase = asm.adm_cc, asm.guard_cc, asm.pace.phase
    _scc = asm.spec.connector_clocking
    eng_clock = np.radians(asm.spec.run.engage_clock_deg)
    pe_frame = str(asm.spec.run.post_engage_frame or 'target').lower()
    cc_sweep, cc_legs, cc_lo, cc_hi, cc_mid = asm.cc_geometry
    cc_push_m = float(_scc.push_mm) / 1000.0
    cc_need_m = float(_scc.success_advance_mm) / 1000.0
    cc_tries = max(1, int(_scc.max_tries))
    cc_settle = asm.settle_s if _scc.settle_s is None else float(_scc.settle_s)
    cc_hold = float(_scc.hold_after_s)
    cc_open_after = bool(_scc.open_gripper_after)
    cc_v, cc_w = _scc.speed_translation_mm_s, _scc.speed_rotation_deg_s
    ...
    # guard would only see force ADDED by the screw. Off, it sees absolute force, at the cost
    # of the spring yielding to the standing load during warm-up.
    cc_tare = asm.tare if _scc.tare_before else None
    T_tool0_engaged = robot.tool0()
    T_tool0_conn = inverse(T_tool0_engaged) @ frames_t.T_clk
    moved = matrix_to_xyzrpy(inverse(robot.T_tool0_fingertip @ asm.T_ftip_conn) @ T_tool0_conn)
    ...
    def arm_at(th_rel, push):
        ...
        return (frames_t.T_clk
                @ xyzrpy_to_matrix([push, 0.0, 0.0], [th_rel, 0.0, 0.0])
                @ inverse(frames_t.T_clk)) @ ref_start
    ...
    if bad:
        log.warning('  CONNECTOR CLOCKING: %d of 25 poses sampled across the %.0f deg band have '
                    ...
                    'move connector_clocking.sweep_deg, or the engage angle, rather than '
                    'letting a leg discover it mid-turn.',
                    len(bad), np.degrees(cc_hi - cc_lo), np.degrees(eng_clock + bad[0]))

    det = _ScrewAdvance(robot, T_tool0_conn, frames_t.T_clk, cc_need_m)
    ...
    got = matrix_to_xyzrpy(inverse(frames_t.T_clk) @ T_base_conn)
...
def collar_clocking(asm, T_base_conn, screw_deg=None, cl_rot=None, sp_on=None,
                    unlocking=False):
    ...
    robot, frames_t, clock_rows = asm.robot, asm.frames, asm.clock_rows
    adm_cl, guard_cl = asm.adm_cl, asm.guard_cl
    guard_push, guard_shared, tare = asm.guard_push, asm.guard, asm.tare
    _guarded = guarded
    phase, seg_time, scales = asm.pace.phase, asm.pace.seg_time, asm.pace.scales
    g_v, g_w, s_std = asm.pace.g_v, asm.pace.g_w, asm.pace.s_std
    _T_tool0_conn = asm.T_tool0_conn_engaged
    _scl = asm.spec.collar_clocking
    _ssp = _scl.seat_push
    eng_clock = np.radians(asm.spec.run.engage_clock_deg)
    pe_frame = str(asm.spec.run.post_engage_frame or 'target').lower()
    cl_axis_off = [float(v) for v in (_scl.axis_offset_mm or [0.0, 0.0, 0.0])]
    cl_off_m = float(_scl.collar_offset_mm) / 1000.0
    cl_push_m = float(_scl.push_mm) / 1000.0
    # GRASP CLOCK ANGLE: the ABSOLUTE roll about the socket +X to take the collar at, wrt
    # the TARGET frame -- the same convention connector_clocking.sweep_deg uses. null = the
    # engaged roll. The turn then runs from here to here + rotation_deg. (Replaces
    # prewind_deg, which existed only because the old radial approach reached the ring by
    # orbiting from wherever the sweep left the arm; an axial approach is placed in free
    # space, so the angle is simply stated.)
    _gc = _scl.grasp_clock_deg
    ...
    cl_retract_m = float(_scl.retract_mm) / 1000.0
    # RETREAT: how far BEHIND the approach station, along the connector -X, to reorient.
    # Away from the wall, and the furthest-from-it station in the whole maneuver.
    cl_retreat_m = float(_scl.retreat_mm) / 1000.0
    # WALL STANDOFF: the largest tool0 station along the connector +X any planned pose may
    # take, mm in the TARGET frame (+X points INTO the wall the socket is mounted on,
    # 0 = the mating face). Checked pre-motion against every station. null = no check.
    cl_wall_mm = None if _scl.wall_standoff_mm is None else float(_scl.wall_standoff_mm)
    cl_settle = asm.settle_s if _scl.settle_s is None else float(_scl.settle_s)
    cl_v, cl_w = _scl.speed_translation_mm_s, _scl.speed_rotation_deg_s
    # TARE: default OFF -- see connector_clocking's head; the same standing-load reasoning.
    cl_tare = asm.tare if _scl.tare_before else None
    cl_tilt_deg = float(_scl.max_offaxis_tilt_deg)
    # WRIST-3 MARGIN: how far inside each end of the usable wrist_3 range the collar turn
    # must stay. The turn is servoed under admittance, so the arm does not track the
    # reference exactly and a start placed hard against the limit can still walk into it.
    cl_w3_margin = np.radians(_scl.wrist3_margin_deg)
    sp_force = float(_ssp.force_n)
    sp_persist = float(_ssp.persistence_s)
    sp_travel_m = float(_ssp.max_travel_mm) / 1000.0
    if cl_rot is None:
        cl_rot = np.radians(_scl.rotation_deg)
    if sp_on is None:
        sp_on = bool(_ssp.enabled)
    ...
    axis = frames_t.T_clk[:3, 0]
    axn = axis / float(np.linalg.norm(axis))
    cammed = float(np.dot(T_base_conn[:3, 3] - frames_t.T_clk[:3, 3], axn))
    point = frames_t.T_clk[:3, 3] + cammed * axn
    ...
    if float(np.linalg.norm(off_conn)) > 0.0:
        point = point + axis_offset_base(asm)
    ...
    T_base_axis = frames_t.T_clk.copy()
    ...
    # belief reset put the connector AT frames_t.T_clk, so the ring's station is a property of the
    ...
    # engagement (`_T_tool0_conn` = the connector expressed in tool0 THERE), and frames_t.T_clk is that
    # same connector in base -- so the flange pose that produced it is frames_t.T_clk @ inverse(belief).
    T_engaged = frames_t.T_clk @ inverse(_T_tool0_conn)
    ...
    if pinned:
        ...
        th_now = float(matrix_to_xyzrpy(inverse(frames_t.T_clk) @ T_reorient)[1][0])
    ...
    x_tool = {lab: float((inverse(frames_t.T_clk) @ T)[0, 3]) * 1000.0 for lab, T in stations}
    ...
    log.info('  (the sweep left the arm at %+.1f mm and the engagement pose is at %+.1f mm '
             ...
             float((inverse(frames_t.T_clk) @ here)[0, 3]) * 1000.0,
             float((inverse(frames_t.T_clk) @ T_engaged)[0, 3]) * 1000.0)
    ...
    if unlocking:
        ...
        adm_cl.warmup(_now)
        guard_shared.reset()
        res_re, f_re = screw_ramp(asm, adm_cl, _to_engaged, guard_shared,
                                             cl_v, cl_w,
...
def screw_ramp(asm, adm, ref_at, guard, v, w, ang_deg, label=''):
    ...
    if chord_mm > 1e-6:
        ...
        sag_mm = max(float(np.linalg.norm((p - pts[0]) - np.dot(p - pts[0], u) * u))
                     for p in pts) * 1000.0
    cv, cw = asm.pace.caps(v, w)  # a null override must be resolved BEFORE the arithmetic
    dur = path_time(arc_mm, ang_deg, cv, cw, asm.pace.min_seg_s)
...
def run_insertion(asm, adm_ctl, refs, T_tool0_conn, peck=False, guard_ctl=None,
                  settle=None, hold=None, speed=None, pause=None, retract=True):
    ...
    guard = guard_ctl if guard_ctl is not None else asm.guard
    settle_s = settle if settle is not None else asm.settle_s
    hold_s = hold if hold is not None else asm.hold_s
    sv, sw = speed if speed is not None else (None, None)
    p, decim = asm.pace, max(1, int(asm.spec.run.log_decimation))
    obs, cnt = [], [0]

    def log_cb():
        cnt[0] += 1
        if cnt[0] % decim == 0:
            obs.append(observe(asm.robot, T_tool0_conn, asm.frames.T_base_tconn))

    adm_ctl.reset()
    adm_ctl.warmup(refs[0], tare_fn=asm.tare)
    guard.reset()
    last_ref, i, t0, stops = refs[0], 1, _time.time(), []
    prev = refs[0]
    while i < len(refs):
        res = adm_ctl.ramp(prev, refs[i], p.seg_time(prev, refs[i], sv, sw), guard,
                           on_step=log_cb)
        last_ref = refs[i]
        if res == 'seated':
            stops.append(float(matrix_to_xyzrpy(
                inverse(asm.frames.T_base_tconn) @ asm.robot.tool0() @ T_tool0_conn)[0][0] * 1000.0))
            if not peck or _time.time() - t0 > float(asm.spec.collection.peck_timeout_s):
                break
            ...
            T_out = retract_ref(last_ref, T_tool0_conn,
                                float(asm.spec.collection.peck_retract_mm) / 1000.0)
            adm_ctl.ramp(last_ref, T_out, p.seg_time(last_ref, T_out, p.g_v * p.s_ret,
                                                     p.g_w * p.s_ret), guard=None,
                         on_step=log_cb)
            guard.reset()
            prev = T_out
            i += 1
...
def retract_from(asm, last_ref, T_tool0_conn, adm_ctl=None):
    ...
    ctl = adm_ctl if adm_ctl is not None else asm.adm
    p = asm.pace
    T_out = retract_ref(last_ref, T_tool0_conn,
                        float(asm.spec.run.retract_distance_mm) / 1000.0)
    ctl.ramp(last_ref, T_out,
             p.seg_time(last_ref, T_out, p.g_v * p.s_ret, p.g_w * p.s_ret), guard=None)
    asm.robot.arm.servo_stop()
    return T_out


def retract_along_target(asm, last_ref, distance_m):
    ...
    p = asm.pace
    axis = np.asarray(asm.frames.T_base_tconn[:3, 0], dtype=float)
    axis = axis / max(float(np.linalg.norm(axis)), 1e-12)
    T_out = translation_matrix(-float(distance_m) * axis) @ np.asarray(last_ref, dtype=float)
    asm.adm.ramp(last_ref, T_out,
             p.seg_time(last_ref, T_out, p.g_v * p.s_ret, p.g_w * p.s_ret), guard=None)
    asm.robot.arm.servo_stop()
    return T_out


def clocking_retract(asm, label='clocking retract', gripper_leg=True):
    ...
    def leg(vec, dist, in_target):
        ...
        if in_target:
            ...
            T = translation_matrix(asm.frames.T_clk[:3, :3] @ step) @ asm.robot.tool0()
            what = f'target {np.round(v / n, 3).tolist()}'
        else:
            # a direction in the GRIPPER's own (tool0) frame -> right-multiply
            T = asm.robot.tool0() @ translation_matrix(step)
            what = f'gripper {np.round(v / n, 3).tolist()}'
        return guarded(asm.robot, asm.guard, lambda: asm.robot.arm.move_l(
            T, label=f'{label} ({what}, {abs(float(dist)) * 1000.0:.0f} mm)'))

    asm.pace.phase('clock_retract')
    if not gripper_leg:
    ...
    return ((gripper_leg is False
             or leg(r.gripper_axis,
                    float(r.gripper_distance_mm) / 1000.0, False))
            and leg(r.target_axis,
                    float(r.target_distance_mm) / 1000.0, True))


def phase_gate(asm, name, ahead):
    ...
    if asm.robot.arm.dry_run or not asm.gates_on:
        return True
    try:
...
def end_reset_with_snapshot(asm, label='end reset'):
    ...
    ok = reset.reset_robot(asm.robot, asm.cfg, label)
    if (ok and asm.camera is not None and not asm.robot.arm.dry_run
            and bool(asm.cfg.get('end_view_image', True))):
        try:
            import cv2
            frame = asm.camera.capture()
            path = os.path.join(asm.out_dir, 'end_view.jpg')
...
def axis_offset_base(asm):
    ...
    off = [float(v) for v in (asm.spec.collar_clocking.axis_offset_mm or [0.0, 0.0, 0.0])]
    return (asm.frames.T_clk[:3, :3] @ asm.frames.R_clock[:3, :3].T) @ (
        np.asarray(off, dtype=float) / 1000.0)


def place_scatter(asm):
    ...
    sc = asm.spec.disassembly.place_scatter
    if not sc.enabled:
        return 0.0, 0.0, 0.0
    rx = abs(float(sc.x_mm)) / 1000.0
    ry = abs(float(sc.y_mm)) / 1000.0
    rw = np.radians(abs(float(sc.yaw_deg)))
    rng = asm.place_rng[0]
    return (float(rng.uniform(-rx, rx)), float(rng.uniform(-ry, ry)),
            float(rng.uniform(-rw, rw)))


def aligned_place_pose(asm, extra_clearance_m=0.0, scatter=(0.0, 0.0, 0.0)):
    ...
    p = asm.frames.T_base_tconn[:3, 3] + asm.frames.T_base_tconn[:3, :3] @ d  # offsets in the TARGET frame
    yaw = (float(np.arctan2(asm.frames.T_base_tconn[1, 0], asm.frames.T_base_tconn[0, 0]))
    ...
    axis_z = asm.ground_z + connector_axis_height_m(asm.cfg) + float(extra_clearance_m)
    if bool(r.snap_to_ground):
        log.info('REORIENT PLACE: z_mm %+.0f puts the connector axis %+.0f mm above the '
                 ...
                 'radius, %.1f mm, up).', z_mm, (p[2] - asm.ground_z) * 1000.0,
                 (axis_z - asm.ground_z) * 1000.0, connector_axis_height_m(asm.cfg) * 1000.0)
        p[2] = axis_z
    T[:3, 3] = p
    return T


def tug_verify_in_place(asm, hold_after=False):
    ...
    robot, cfg, frames_t = asm.robot, asm.cfg, asm.frames
    adm_tug, guard_shared, tare = asm.adm_tug, asm.guard, asm.tare
    phase, seg_time, clock_rows = asm.pace.phase, asm.pace.seg_time, asm.clock_rows
    tv = asm.spec.tug_verify
    tv_force, tv_time = float(tv.pull_force_n), float(tv.pull_time_s)
    tv_thresh_m = float(tv.displacement_threshold_mm) / 1000.0
    tv_extract_m = float(tv.extraction_distance_mm) / 1000.0
    dis_on = bool(asm.spec.disassembly.enabled)
    axn_t = frames_t.T_clk[:3, 0] / float(np.linalg.norm(frames_t.T_clk[:3, 0]))
    ...
    if held:
        ...
        return ('verified' if clocking_retract(asm, 'tug retract', gripper_leg=not dis_on)
                else 'error')
...
def place_after_failed_engage(asm, miss_ref, T_tool0_conn_held):
    ...
    robot, frames_t, guard_shared = asm.robot, asm.frames, asm.guard
    phase, _guarded = asm.pace.phase, guarded
    no_prompts, q_pick = asm.no_prompts, asm.q_pick
    fr = asm.spec.engage.fail_retract
    fail_retract_m = float(fr.distance_mm) / 1000.0
    fail_retract_min_m = float(fr.min_mm) / 1000.0
    dp = asm.spec.disassembly.place
    dis_clear_m = float(dp.clearance_mm) / 1000.0
    dis_rise_m = float(dp.retreat_mm) / 1000.0
    ...
    def conn_depth_mm():
        return float((inverse(frames_t.T_base_tconn)
    ...
    try:
        retract_along_target(asm, miss_ref, fail_retract_m)
...
def locate_target_visually(asm, q_return):
    ...
    robot, camera, cfg, frames_t = asm.robot, asm.camera, asm.cfg, asm.frames
    phase, out_dir = asm.pace.phase, asm.out_dir
    targets, tname, vt_rig = asm.targets, asm.tname, asm.vt_rig
    vt = asm.spec.visual_target
    from ..perception import ArucoDetector

    if camera is None:
        log.error('run.target_source is visual but the app has no camera.')
        return False
    try:
    ...
    if not phase_gate(asm, 'VISUAL LOCALIZATION',
    ...
    if q_return is not None and vt.return_home_after:
    ...
    if T_vis is None:
        log.error('VISUAL TARGET: the markers did not produce a target pose. Nothing has '
                  ...
                  'run.target_source: kinematic to use it, or fix the rig.')
        return False
    ...
    lim_mm = vt.max_shift_mm
    lim_deg = vt.max_shift_deg
    ...
    (log.error if over else log.info)(
        ...
        ' -- OVER visual_target.max_shift_mm/_deg (%s mm / %s deg)'
...
def reorient_recovery(asm):
    ...
    robot, cfg, frames_t, guard_shared = asm.robot, asm.cfg, asm.frames, asm.guard
    phase, _guarded = asm.pace.phase, guarded
    scanner, geom, check = asm.scanner, asm.geom, asm.check
    recovery, grasp, confirm, recorder = asm.recovery, asm.grasp, asm.confirm, asm.recorder
    T_ftip_conn_catalogue = asm.T_ftip_conn_catalogue
    r = asm.spec.reorient_recovery
    if not bool(r.enabled):
        log.error('The coaxial grasp is unreachable and reorient_recovery is off.')
        return False
    if not phase_gate(asm, 'REORIENT THE CABLE',
    ...
    square = dict(r.fingertip_in_connector
    ...
    try:
        ...
        T_place = aligned_place_pose(
            asm, float(r.release_clearance_mm) / 1000.0)
        ...
        above = translation_matrix(
            [0.0, 0.0, float(r.approach_mm) / 1000.0])
...
def celebrate(asm, state, cycle, tug_res, ret_ok):
    ...
    if tug_res not in (None, 'skipped', 'verified'):
        ...
        return True
    want = 'locked' if asm.cl_on else ('seated' if asm.cc_on else 'engaged')
    if state != want:
    ...
    when = str(cb.when).strip().lower()
    every_n = max(1, int(cb.every_n))
    if when in ('last', 'end'):
        due = (cycle >= asm.n_cycles)
    elif when in ('every', 'each', 'cycle'):
        due = True
    elif when in ('every_n', 'nth'):
        due = (cycle % every_n == 0)
    else:
        log.warning("CELEBRATE: celebrate.when=%r is not one of "
                    "last | every | every_n -- treating it as 'last'.", when)
        due = (cycle >= asm.n_cycles)
    if not due:
        log.info('CELEBRATE skipped: cycle %d/%d, when=%r%s.', cycle, asm.n_cycles, when,
                 f' (every {every_n})' if when in ('every_n', 'nth') else '')
        return True

    rise_m = float(cb.rise_mm) / 1000.0
    nod_deg = float(cb.nod_deg)
    spin_deg = float(cb.spin_deg)
    reps = max(1, int(cb.repeats))
    flourish = bool(cb.gripper_flourish)
    ...
    asm.pace.phase('retract')

    if rise_m > 0.0:
        T_up = translation_matrix([0.0, 0.0, rise_m]) @ asm.robot.tool0()
        if not guarded(asm.robot, asm.guard,
                        lambda: asm.robot.arm.move_l(T_up, label='celebrate (rise)')):
            log.warning('CELEBRATE: could not rise -- skipping the rest. The assembly is '
                        'unaffected.')
            return True

    model = asm.grasp.collision_model()
    q0 = list(asm.robot.arm.q())
    ...
    for _ in range(reps):
        ...
        if not wiggle_joint(UR_JOINTS.index('wrist_3_joint'), spin_deg, 'spin'):
            break
        if flourish:
            asm.robot.gripper.close('celebrate (clap)')
            asm.robot.gripper.open('celebrate (clap)')
