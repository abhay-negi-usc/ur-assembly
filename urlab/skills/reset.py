...
def go_home(robot, cfg, guard=None):
    """Return to the home joint config under admittance. Returns True on a clean home, False if the
    guard tripped (hit something) or a move failed."""
    ...
    if cfg.get_path('reset.compliance.tare_before', True):
...
def reset_robot(robot, cfg, confirm=None, label='reset'):
    """Open the gripper, then return home under admittance. The whole-run bookend."""
    if confirm and not confirm(f'{label}: open gripper + go home (admittance)'):
        return False
